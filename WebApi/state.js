function MenuState(name) {
	...
	var _yPos = 100;
	btns.push(new MenuButton("Un Jugador", 20, _yPos, function() {
		state.get("game").init(ONE_PLAYER);
		state.change("game");
	}));
	btns.push(new MenuButton("2 Jugadores", 20, _yPos+70, function() {
	...
	this.render = function(_ctx) {
		...
		ctx.fillStyle = "#FB380E";
		var txt = "Juego de Triqui";
...
function GameState(name) {
	...
	this.update = function() {
		...
		if (!activeAnim) {
			...
			if (winner && !aiMoved) {
				if (winner === true) {
					winnerMsg = "El Juego es un Empate!";
				} else if (winner === Tile.NOUGHT) {
					winnerMsg = "!El Jugador Del Circulo Gano!";
				} else {
					winnerMsg = "¡El jugador de la Cruz ganó!";
				}
			}
